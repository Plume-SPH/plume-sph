...
//   */
//  int loop = 0;
//  for (loop = 0; loop < num_loop_P; loop ++)
//  {
//	  itr2->reset();
...
search_neighs_consth (int myid, THashTable * P_table, HashTable * BG_mesh)
{
  ...
  while ((curr_bucket = (Bucket *) igrd->next()))
  {
    ...
    if (!curr_bucket->is_guest() && (curr_bucket->get_plist().size() > 0))
    {
      vector < TKey > plist = curr_bucket->get_plist();
      vector < TKey >::iterator itr;
      if (curr_bucket->has_wall_ghost_particles () || curr_bucket->has_pressure_ghost_particles () || curr_bucket->has_pressure_ghost_particles()) //add a if else statement here to avoid unnecessary if statement for buckets that does not have any ghost particles
      {
    	  for (itr = plist.begin(); itr != plist.end(); itr++)
    	  {
    	     pi = (Particle *) P_table->lookup(*itr);
